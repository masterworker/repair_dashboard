...
# Load the dataset
file_path = "https://raw.githubusercontent.com/masterworker/repair_dashboard/main/final_data.csv"
...
if missing_columns:
    ...
    if selected_tab == "수리 내역 대시보드":
        ...
        df_filtered.to_csv(r'C:\Users\voice\Desktop\공모전\이륜차 자동보급\교체검토차량.csv', index=False, encoding='utf-8-sig')
        ...
        # Option to download the replacement list
        csv = replacement_list.to_csv(index=True).encode('utf-8-sig')
        st.download_button(
            label="교체 대상 목록 다운로드",
            data=csv,
            file_name='교체대상_목록.csv',
            mime='text/csv'
        )
